...
var availabilityChart = dc.pieChart("#availability-chart");
...
var currentCpiNumberDisplay = dc.numberDisplay("#current-cpi-number-display");
...
d3.csv("ndx.csv", function (data) {
    ...
    var yearlyPerformanceGroup = yearlyDimension.group().reduce(
        ...
        function (p, v) {
            ...
            console.log(v);
            p.sumCpi += +v.sum_cpi;
            p.countCpi += +v.count_cpi;
            p.cpi = p.sumCpi / p.countCpi;
            return p;
        },
        ...
        function () {
            return {
                count: 0, absGain: 0,
                fluctuation: 0, fluctuationPercentage: 0,
                sumIndex: 0, avgIndex: 0,
                percentageGain: 0,
                sumCpi: 0, countCpi: 0, cpi: 0
            };
        }
    );
    ...
    var pricesByMonthGroup = moveMonths.group().reduceSum(function (d) {
        return d.count_cpi;
    });
    ...
    var availabilityData = ndx.dimension(function (d) {
        return d.availability;
    });
    // produce counts records in the dimension
    var availabilityDataGroup = availabilityData.group();
    ...
    availabilityChart
        ...
        .dimension(availabilityData) // set dimension
        .group(availabilityDataGroup) // set group
        /* (optional) by default pie chart will use group.key as it's label
         * but you can overwrite it with a closure */
        .label(function (d) {
            if (availabilityChart.hasFilter() && !availabilityChart.hasFilter(d.data.key))
                return d.data.key + "(0%)";
